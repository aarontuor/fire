...
def bidir_lm_rnn(x, t, token_embed, layers, seq_len=None, context_vector=None, cell=tf.nn.rnn_cell.BasicLSTMCell):
    ...
    f_hidden_states, b_hidden_states = tf.split(tf.stack(hidden_states), 2,
                                                axis=2)  # 2  sen_len X num_users X hidden_size tensors
    ...
    prediction_states = tf.unstack(
        tf.concat((f_hidden_states, b_hidden_states), 2))  # sen_len-2 long list of num_users X 2*hidden_size tensors
...
def batch_softmax_dist_loss(truth, h, dimension, scale_range=1.0):
    ...
    loss_matrix = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                 labels=truth)  # batch_size X sequence_length
    return loss_matrix
...
def full_mvn_loss(truth, h, scale_range=1.0):
    ...
    #                                                              dtype=tf.float32,
    #                                                              name='U'))
    # var2 = 0.001*tf.truncated_normal([256, dimension, dimension],
    #                                 dtype=tf.float32,
    #                                 name='U')
    # var = tf.matrix_band_part(var2, -1, 0)

    b = tf.Variable(tf.zeros([dimension + dimension ** 2]))
    y = tf.matmul(h, U) + b
    mu = tf.slice(y, [0, 0], [-1, dimension])  # is MB x dimension
    var = tf.slice(y, [0, dimension],
                   [-1, -1]) * 0.0001  # is MB x dimension^2 # WARNING WARNING TODO FIX THIS MAGIC NUMBER
    var = tf.reshape(var, [-1, dimension,
                           dimension])  # make it a MB x D x D tensor (var is a superset of the lower triangular part of a Cholesky decomp)
    ...
    z = tf.squeeze(tf.matrix_triangular_solve(var, tf.reshape(truth - mu, [-1, dimension, 1]), lower=True,
    ...
    logdet = tf.reduce_sum(tf.log(tf.square(tf.matrix_diag_part(var))),
                           1)  # diag_part converts MB x D x D to MB x D, square and log preserve, then sum makes MB x 1
    ...
    for i, (input_name, loss_func, dimension) in enumerate(loss_spec):
        with tf.variable_scope(input_name):
            ...
            if loss_func == softmax_dist_loss:  # discrete
                ...
                component_wise_point_loss, logdet = loss_func(x, h,
                                                              variance_floor=variance_floor)  # MB X DIM_MULTIVARIATE, MB X 1
...
def true_bptt_rnn(cell, inputs, initial_state=None, dtype=None,
                  sequence_length=None, scope=None, state_index=1):  ### From Tensorflow
    """
...
class IdentRANCell(RANCell):
    """
    Defaults to use identity instead of tanh activation.
    """
...
class GRUCell(tf.nn.rnn_cell.GRUCell):
    """
    Adapted GRUCell to take and return LSTMStateTuples for compatibility with our code.
    """

    def __call__(self, inputs, state, scope=None):
        if type(state) is tf.contrib.rnn.LSTMStateTuple:
            state = state.c
        output, new_state = super(GRUCell, self).__call__(inputs, state, scope=scope)
        return output, tf.contrib.rnn.LSTMStateTuple(new_state, new_state)
